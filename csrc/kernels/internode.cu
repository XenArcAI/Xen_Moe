    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        if (lane_id < kNumRDMARanks) {
            while (true) {
                ...
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                    printf("Xen-MoE dispatch forwarder timeout (RDMA meta), channel: %d, RDMA: %d, nvl: %d, src RDMA lane: %d, dst NVL: %d, meta: %d, %d, %d, %d\n",
        ...
        while (__any_sync(0xffffffff, num_tokens_to_recv_from_rdma > 0)) {
            ...
            while (lane_id == 0) {
                ...
                // Timeout check
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                    printf("Xen-MoE dispatch forwarder timeout (NVL check), channel: %d, RDMA: %d, nvl: %d, dst NVL: %d, head: %d, tail: %d\n",
            ...
            while (true) {
                ...
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES and lane_id < kNumRDMARanks) {
                    printf("Xen-MoE dispatch forwarder timeout (RDMA check), channel: %d, RDMA: %d, nvl: %d, dst NVL: %d, src RDMA lane: %d, head: %d, tail: %d, expected: %d\n",
        ...
        while (lane_id < kNumRDMARanks) {
            ...
            // Timeout check
            if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                printf("Xen-MoE dispatch NVL receiver timeout, channel: %d, RDMA: %d, nvl: %d, src RDMA: %d, src nvl: %d, start: %d, end: %d\n",
        ...
        while (num_tokens_to_recv > 0) {
            ...
            while (lane_id == 0) {
                ...
                // Timeout check
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                    printf("Xen-MoE dispatch NVL receiver timeout, channel: %d, RDMA: %d, nvl: %d, src NVL: %d, head: %d, tail: %d\n",
    ...
    if (warp_role == WarpRole::kNVLSender) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                // Timeout check
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES and lane_id < kNumRDMARanks) {
                    printf("Xen-MoE combine NVL sender timeout, channel: %d, RDMA: %d, nvl: %d, dst NVL: %d, RDMA lane: %d, head: %d, tail: %d, start: %d, end: %d\n",
        ...
        if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
            ...
            for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine; token_start_idx += num_max_rdma_chunked_send_tokens) {
                ...
                while (sub_warp_id == 0 and lane_id == 0) {
                    ...
                    // Timeout check
                    if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                        printf("Xen-MoE combine forwarder (RDMA check) timeout, channel: %d, RDMA: %d, nvl: %d, dst RDMA: %d, head: %ld, tail: %d, chunked: %d\n",
                ...
                for (int token_idx = token_start_idx + sub_warp_id; token_idx < token_end_idx; token_idx += kNumWarpsPerForwarder) {
                    ...
                    while (cached_nvl_channel_tail_idx <= expected_head) {
                        ...
                        if (clock64() - start_time > NUM_TIMEOUT_CYCLES and lane_id < NUM_MAX_NVL_PEERS) {
                            printf("Xen-MoE combine forwarder (NVL check) timeout, channel: %d, RDMA: %d, nvl: %d, src NVL: %d, dst RDMA: %d, tail: %d, waiting: %d, total: %d, sub: %d, large: %d, expected: %d\n",
            ...
            for (int64_t token_idx = token_start_idx + warp_id; token_idx < token_end_idx; token_idx += kNumRDMAReceivers) {
                ...
                while (cached_channel_tail_idx <= expected_head) {
                    ...
                    // Timeout check
                    if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                        printf("Xen-MoE combine RDMA receiver timeout, channel: %d, RDMA: %d, nvl: %d, src RDMA: %d, tail: %d, waiting: %ld, expect: %d\n",
