    ...
    if (is_sender) {
        ...
        for (int64_t token_idx = token_start_idx; token_idx < token_end_idx; ) {
            ...
            while (send_lane_id == 0) {
                ...
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                    printf("Xen-MoE timeout for dispatch senders, rank %d, responsible_channel = %d\n", rank, responsible_channel);
                    trap();
                }
            }
            __syncwarp();
        ...
        while (num_tokens_to_recv > 0) {
            ...
            while (recv_thread_id_in_rank == 0) {
                ...
                // Timeout check
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                    printf("Xen-MoE timeout for dispatch receivers, rank %d, responsible_channel = %d, tokens remained: %d\n", rank, responsible_channel, num_tokens_to_recv);
                    trap();
                }
            }
    ...
    if (is_sender) {
        ...
        for (int64_t token_idx = token_start_idx; token_idx < token_end_idx; ) {
            ...
            while (send_lane_id == 0) {
                ...
                // Rare cases to loop again
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                    printf("Xen-MoE timeout for combine senders, rank %d, responsible_channel = %d\n", rank, responsible_channel);
                    trap();
                }
            }
            __syncwarp();
        ...
        if (thread_id < 32) {
            ...
            for (int64_t token_idx = token_start_idx + recv_warp_id - 1; token_idx < token_end_idx; token_idx += num_recv_warps - 1) {
                ...
                while (channel_tail_idx[recv_lane_id] <= expected_head and expected_head >= 0) {
                    // Timeout check
                    if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                        printf("Xen-MoE timeout for combine receivers, rank %d, responsible_channel = %d, expect = %d\n", rank, responsible_channel, expected_head);
                        trap();
                    }
                }
                __syncwarp();
