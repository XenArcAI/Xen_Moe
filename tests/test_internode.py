...
def test_main(num_sms: int, local_rank: int, num_local_ranks: int, num_ranks: int, num_nodes: int, rank: int, buffer: xen_moe.Buffer, group: dist.ProcessGroup):
    # Settings
    ...
    config = xen_moe.Config(num_sms, 8, nvl_buffer_size, 16, rdma_buffer_size)
    ...
    for current_x in (x_e4m3, x):
        ...
        for nvl_chunk_size in range(4, 33, 4):
            for rdma_chunk_size in range(4, 33, 4):
                config = xen_moe.Config(num_sms, nvl_chunk_size, nvl_buffer_size, rdma_chunk_size, rdma_buffer_size)
    ...
    dispatch_config = xen_moe.Config(best_dispatch_results[0], best_dispatch_results[1], nvl_buffer_size, best_dispatch_results[2], rdma_buffer_size)
...
def test_loop(local_rank: int, num_local_ranks: int):
    ...
    buffer = xen_moe.Buffer(group, int(1e9), int(1e9), low_latency_mode=test_ll_compatibility,
