...
def test_main(num_sms: int, local_rank: int, num_ranks: int, rank: int, buffer: xen_moe.Buffer, group: dist.ProcessGroup):
    # Settings
    ...
    # Config
    nvl_buffer_size = 256
    config = xen_moe.Config(num_sms, 8, nvl_buffer_size)
    ...
    for current_x in (x_e4m3, x):
        ...
        for nvl_chunk_size in range(4, 33, 4):
            config = xen_moe.Config(num_sms, nvl_chunk_size, nvl_buffer_size)
    ...
    dispatch_config = xen_moe.Config(best_dispatch_results[0], best_dispatch_results[1], nvl_buffer_size)
...
def test_loop(local_rank: int, num_local_ranks: int):
    ...
    if test_ll_compatibility:
        ...
        num_rdma_bytes = xen_moe.Buffer.get_low_latency_rdma_size_hint(ll_num_tokens, ll_hidden, num_ranks, ll_num_experts)

    buffer = xen_moe.Buffer(group, int(1e9), num_rdma_bytes, low_latency_mode=test_ll_compatibility,
