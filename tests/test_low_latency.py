...
def test_main(num_tokens: int, hidden: int, num_experts: int, num_topk: int,
              rank: int, num_ranks: int, group: dist.ProcessGroup, buffer: xen_moe.Buffer, seed: int = 0):
...
def test_loop(local_rank: int, num_local_ranks: int):
    ...
    num_rdma_bytes = xen_moe.Buffer.get_low_latency_rdma_size_hint(num_tokens, hidden, num_ranks, num_experts)
    if local_rank == 0:
        print(f'Allocating buffer size: {num_rdma_bytes / 1e6} MB ...', flush=True)
    buffer = xen_moe.Buffer(group, num_rdma_bytes=num_rdma_bytes, low_latency_mode=True,
